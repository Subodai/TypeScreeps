...
/**
 * Resets a creeps memory to start to stop things getting stuck on targets
 */
Creep.prototype.clearTargets = function () {
    let mem = {};
    mem.role = this.memory.role;
    mem.level = this.memory.level;
    mem.sType = this.memory.sType;
    mem.roomName = this.memory.roomName;
    this.memory = mem;
    console.log('[TRAVELER]' + this.name + ' Creep Memory Reset');
}
